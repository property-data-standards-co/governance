...
A question is badly framed if it names a technology, if it can be answered yes or no in a way that settles the architecture, or if one option is the obvious reading of the words. "Should we use verifiable credentials?" imports its answer. "What is the minimum unit of assertion that satisfies R-INDEPENDENCE?" does not.
